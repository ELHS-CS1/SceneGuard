...
class RotateBehavior implements Behavior<AbstractMesh> {
    ...
    constructor(scene: Scene, private _options: RotateBehaviorOptions) {
        this._scene = scene;
    }
    ...
    attach(target: AbstractMesh): void {
        this._scene.onBeforeRenderObservable.add(() => {
            target.rotate(this._options.axis, this._options.speed * this._scene.getEngine().getDeltaTime() / 1000);
        });
    }
...
class ScaleBehavior implements Behavior<AbstractMesh> {
    ...
    constructor(scene: Scene, private _options: ScaleBehaviorOptions) {
        this._scene = scene;
    }
    ...
    attach(target: AbstractMesh): void {
        ...
        this._scene.onBeforeRenderObservable.add(() => {
            ...
            const progress = Math.min((Date.now() - this._startTime) / (this._options.duration * 1000), 1);
...
class MoveBehavior implements Behavior<AbstractMesh> {
    ...
    constructor(scene: Scene, private _options: MoveBehaviorOptions) {
        this._scene = scene;
    }
...
export class BehaviorFactory {
    private readonly _behaviorMap: {
        [K in BehaviorType]: new (scene: Scene, options: BehaviorConfig<K>['options']) => Behavior<AbstractMesh>;
    } = {
